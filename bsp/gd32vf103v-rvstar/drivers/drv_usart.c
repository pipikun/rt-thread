 ...
 * 2021-02-24     carson       change uart typedef
 */
...
struct gd32_usart {
    ...
    struct uart_gpio_typdef  tx;
    struct uart_gpio_typdef  rx;
    IRQn_Type irqn;
...
static rt_err_t gd32_configure(struct rt_serial_device *serial,
    ...
    /* enable GPIO clock */
    rcu_periph_clock_enable(usart->tx.gpio_clk);
    rcu_periph_clock_enable(usart->rx.gpio_clk);
    /* enable USART clock */
    rcu_periph_clock_enable(usart->usart_clk);
    /* connect port to USARTx_Tx */
    gpio_init(usart->tx.gpio_port, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ,
            usart->tx.gpio_pin);
    /* connect port to USARTx_Rx */
    gpio_init(usart->rx.gpio_port, GPIO_MODE_IN_FLOATING, GPIO_OSPEED_50MHZ,
            usart->rx.gpio_pin);
...
void UART4_IRQHandler(void) {
    ...
    usart_isr(&usart_config[GDUART4_INDEX].serial);
